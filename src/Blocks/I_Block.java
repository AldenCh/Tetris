...
public class I_Block extends Block{
	// Sideways position is the right center block: ****
	//										 		  ^
	// Otherwise, position is the center
	...
	public void rotateLeft() {
		tiles.clear();
		if (orientation%2 == 1) {
			Integer[] temp1 = {position[0], position[1]-1};
			tiles.add(temp1);
			Integer[] temp2 = {position[0], position[1]};
			tiles.add(temp2);
			Integer[] temp3 = {position[0], position[1]+1};
			tiles.add(temp3);
			Integer[] temp4 = {position[0], position[1]+2};
			tiles.add(temp4);
		}
		else{
			Integer[] temp1 = {position[0]-2, position[1]};
			tiles.add(temp1);
	...
	public boolean canRotateLeft(GamePanel board) {
		if (orientation%2 == 0) {
			if (position[0]-2 < 0 || position[0]+1 > 8) {
				return false;
			}
			else if (board.getTile(position[0]-2, position[1]) == Color.black || 
			...
			if (position[1]-1 < 0 || position[1]+2 > 17) {
				return false;
			}
	...
	public boolean canMoveDown(GamePanel board) {
		if (orientation%2 == 1){
			...
			else if (board.getTile(tiles.get(0)[0], tiles.get(0)[1]+1) == Color.black ||
					 board.getTile(tiles.get(1)[0], tiles.get(1)[1]+1) == Color.black ||
					 board.getTile(tiles.get(2)[0], tiles.get(2)[1]+1) == Color.black || 
			...
			else if (board.getTile(tiles.get(3)[0], tiles.get(3)[1]+1)== Color.black) {
				return false;
			}
		}
		return true;
		...
		if (orientation%2 == 0) {
			if (position[0] == 8) {
				return false;
					 ...
					 board.getTile(position[0]+1, position[1]+1) == Color.black || 
					 board.getTile(position[0]+1, position[1]+2) == Color.black) {
				return false;
			}
		}
		else {
	...
	public boolean canMoveLeft(GamePanel board) {
		if (orientation%2 == 0) {
			...
			else if (board.getTile(position[0]-3, position[1]) == Color.black) {
				return false;
			}
		}
		return true;
